...
namespace RefCount
{
 ...
 // reference counter for SmartPtrExcel managed objects
struct RefCnt
{
...
template<typename T> struct SmartPtrExcel
{
	 // default constructor
	SmartPtrExcel()
	 :	_ptr(NULL)
	{
	 ...
	 // The initialized SmartPtrExcel constructor increments the reference counter in struct RefCnt.
	SmartPtrExcel(T* p)
	 :	_ptr(p)
	{
		if (p)
	...
	SmartPtrExcel(const SmartPtrExcel& other)
	 :	_ptr(other._ptr)
	{
		if (_ptr)
			++_ptr->_ref_cnt;
...
public:
	...
	struct CellTable
	{
		struct RowBlock
		{
			...
			vector<Row> rows_;
			vector<SmartPtrExcel<CellBlock> > cellBlocks_;
			DBCell dbcell_;
		};
		ULONG Read(const char* data);
		ULONG Write(char* data);
...
public:
	...
	vector<SmartPtrExcel<BasicExcelWorksheet> > yesheets_;	///< Parsed Worksheets.
};
...
private:
	...
	SmartPtrExcel<Formula> _pFormula;
